...
export default {
  async fetch(request,env){
    ...
    if(request.method==="GET" && url.pathname==="/admin/pay"){
      ...
      const state={sessionId:id,paid:true,amount:2501,paidAt:new Date().toISOString()};
      if(env.RAKHI_KV){
      ...
      await sheetLog({event:"payment_confirmed_by_brother",sessionId:id,data:{amount:2501},at:new Date().toISOString()});
    ...
    try{
      ...
      if(body.event==="gift_unlocked"){
        body.data={...(body.data||{}),amount:2501};
        const state={sessionId:body.sessionId,paid:false,unlocked:true,amount:2501,unlockedAt:new Date().toISOString()};
